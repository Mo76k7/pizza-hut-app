...
export default function PaymentModal({ order, paymentMethod, onClose, onSuccess }) {
  ...
  const handleCopy = () => {
    if (navigator.clipboard) {
      navigator.clipboard.writeText(accountInfo.number).then(() => {
        setCopied(true);
  ...
  const handleUpload = async () => {
    if (!file && !transactionId.trim()) {
      setError('Please provide either a Transaction ID or upload a screenshot.');
      return;
    }
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (transactionId.trim()) {
        updatePayload.txn_id = transactionId.trim();
      }
      ...
      >
        ...
        <div style={{ backgroundColor: 'rgba(255,255,255,0.05)', padding: '16px', borderRadius: '12px', marginBottom: '20px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            ...
            >
              {copied ? (
                <><i className="fa-solid fa-check" style={{ color: 'var(--color-success)' }} /> Copied!</>
              ) : (
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ display: 'block', marginBottom: 8, fontSize: 14, color: '#fff' }}>Transaction ID / Reference No.</label>
          <input 
            type="text" 
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={{ display: 'block', marginBottom: 8, fontSize: 14, color: '#fff' }}>Upload Receipt Screenshot</label>
          <input 
            type="file" 
            accept="image/*" 
          ...
          <div style={{ marginTop: 6, fontSize: 12, color: 'var(--color-text-muted)' }}>
            * Please provide either a Transaction ID or a receipt screenshot.
          </div>
        </div>
        ...
        >
          {loading ? <><i className="fa-solid fa-spinner fa-spin" /> Submitting...</> : <><i className="fa-solid fa-cloud-arrow-up" /> Submit Payment</>}
        </button>
      </div>
    </div>
  );
